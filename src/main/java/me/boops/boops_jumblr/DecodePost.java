...
public class DecodePost {
	...
	public void decode(String post){
		...
		//Decode Trail vars
		if(json.has("trail")){
			JSONArray sub_trail = json.getJSONArray("trail");
			if(!sub_trail.isNull(0)){
				JSONObject trail = json.getJSONArray("trail").getJSONObject(0);
				if(trail.has("content_raw")){
					this.tumblr_trail_content_raw = trail.getString("content_raw");
				}
				if(trail.has("is_root_item")){
					this.tumblr_trail_is_root = trail.getBoolean("is_root_item");
				}
				if(trail.has("post")){
					this.tumblr_trail_post_id = trail.getJSONObject("post").getLong("id");
				}
				if(trail.has("content")){
					this.tumblr_trail_content = trail.getString("content");
				}
				
				//Set Blog Strings
				JSONObject blog = trail.getJSONObject("blog");
		...
		if(this.tumblr_type.toLowerCase().equals("photo")){
			...
			this.tumblr_photo_org_width = json.getJSONArray("photos").getJSONObject(0).getJSONObject("original_size").getInt("width");
			this.tumblr_photo_org_height = json.getJSONArray("photos").getJSONObject(0).getJSONObject("original_size").getInt("height");
			this.tumblr_photo_org_url = json.getJSONArray("photos").getJSONObject(0).getJSONObject("original_size").getString("url");
			...
			for(int i=0; json.getJSONArray("photos").getJSONObject(0).getJSONArray("alt_sizes").length()>i; i++){
				this.tumblr_photo_alt_widths.add(json.getJSONArray("photos").getJSONObject(0).getJSONArray("alt_sizes").getJSONObject(i).getInt("width"));
				this.tumblr_photo_alt_urls.add(json.getJSONArray("photos").getJSONObject(0).getJSONArray("alt_sizes").getJSONObject(i).getString("url"));
				this.tumblr_photo_alt_heights.add(json.getJSONArray("photos").getJSONObject(0).getJSONArray("alt_sizes").getJSONObject(i).getInt("height"));
			}
		}
		...
		if(this.tumblr_type.toLowerCase().equals("chat")){
			...
			for(int i=0; this.tumblr_chat_dialogue_length>i; i++){
				this.tumblr_chat_dialogue_phrase.add(json.getJSONArray("dialogue").getJSONObject(i).getString("phrase"));
				this.tumblr_chat_dialogue_name.add(json.getJSONArray("dialogue").getJSONObject(i).getString("name"));
				this.tumblr_chat_dialogue_label.add(json.getJSONArray("dialogue").getJSONObject(i).getString("label"));
			}
